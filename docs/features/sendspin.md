...
!!! warning "本 fork 里没有 Sendspin"

    这页描述的是**上游项目**的功能。本仓库这棵树里**没有任何 Sendspin 实现**：
    `main/` 与 `components/` 全文 grep `sendspin` **0 命中**，也没有对应的 Kconfig
    开关。因此本页写的任何步骤在本 fork 上都**做不到**，包括那个播放器角色。
    留档只是为了以后与上游对齐。
