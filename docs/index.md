...
- **Bluetooth A2DP** — receive audio from phones and tablets (classic-BT ESP32 boards only;
  **not** available on this fork's ESP32-S3 target, which has no classic Bluetooth)
- **W5500 Ethernet** — wired networking with automatic WiFi failover
- **Web configuration** — set WiFi and device name from a browser
- **OTA updates** — 本 fork 已恢复（双 4MB 槽 + 系统页上传），但**默认关闭**：
  没设升级口令时接口一律 403；USB 烧录始终可用
