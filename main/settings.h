...
// 升级口令长度上限（与 NVS 里 wifi_pass 同档，够长也够短）
#define SETTINGS_OTA_PASSWORD_MAX 64
...
/**
 * 固件升级口令（网页 /api/ota/update 要用）。
 *
 * ⚠️ **空串是有效值**：表示"没有口令 = 网页升级关闭"。所以 getter 永远返回
 * ESP_OK 且不给默认值，调用方按"空 = 禁用"处理；任何地方都不要把它写进日志。
 *
 * @param out Output buffer for the passphrase ('' when unset)
 * @param len Size of out
 */
esp_err_t settings_get_ota_password(char *out, size_t len);

/**
 * Save (or clear, with "") the upgrade passphrase.
 * @param password Up to SETTINGS_OTA_PASSWORD_MAX bytes; "" disables OTA
 */
