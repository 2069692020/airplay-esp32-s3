...
/* Ensure both conversion buffers hold at least the requested number of samples.
 *
 * Allocates the replacement before freeing the old buffer: a failed malloc must
 * leave the previously working buffer (and its capacity) untouched, otherwise
 * the caller's next dereference is a NULL write AND every later call with a
 * capacity no larger than the failed request silently skips the retry. */
static bool ensure_float_bufs(size_t in_samples, size_t out_samples) {
  if (in_samples > float_in_cap) {
    float *grown = malloc(in_samples * sizeof(float));
    if (!grown) {
      return false;
    }
    free(float_in);
  ...
  if (out_samples > float_out_cap) {
    float *grown = malloc(out_samples * sizeof(float));
    if (!grown) {
      return false;
    }
    free(float_out);
...
size_t audio_resample_process(const int16_t *in, size_t in_frames, int16_t *out,
  ...
  if (!ensure_float_bufs(in_samples, out_samples)) {
    /* Out of memory: drop this frame rather than write through a NULL buffer.
     * The caller sees fewer decoded samples and the timing engine treats the
     * gap as a late/underrun frame, which is recoverable. */
    return 0;
  }
