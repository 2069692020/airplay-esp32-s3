...
bool audio_output_is_active(void) { return playback_task_handle != NULL; }
...
void audio_output_start(void) {
  ...
  // ⚠️ 丢掉悬置的 flush。audio_output_flush() 只置标志，而**只有** playback_task
  //    会清它 —— 本地音源占着 I2S (任务被停掉) 期间 AirPlay 会话里的 FLUSH 就
  //    一直挂着，等这里把任务拉起来，它干的头一件事就是对着刚使能、正要播首帧
  //    的 I2S 再来一次 disable/enable：会话起始处凭空多一次 DMA 复位。
  flush_requested = false;
...
esp_err_t audio_output_write(const void *data, size_t bytes, TickType_t wait) {
  // ⚠️ 这条是 CLAUDE.md 里那条约定的**代码版**: 外部音源必须先 audio_output_stop()
  //    再来写 ("only ever audio_output_write, never audio_output_start")。以前只
  //    写在注释里，违反的后果是两个任务交替写同一个 DMA —— §25 的"顿+杂音"和
  //    蓝牙接管那条路都是它。这里直接拒掉，让违规变成看得见的一次错误返回，
  //    而不是持续劣化的音质。
  //    AirPlay 自己的 playback_task 走 i2s_channel_write()，不经过这里。
  if (playback_task_handle != NULL) {
    static uint32_t s_denied = 0;
    if ((s_denied++ % 200u) == 0u) {
      ESP_LOGE(TAG,
               "拒绝外部写入: AirPlay playback_task 仍在写 I2S (累计 %" PRIu32
               " 次) —— 先调 audio_output_stop()",
               s_denied);
    }
