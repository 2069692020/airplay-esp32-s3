...
/**
 * True when the AirPlay playback task is running (i.e. audio_output_start()
 * was called and audio_output_stop() has not yet completed).
 *
 * External sources (web radio) use this to decide whether it is safe to
 * reconfigure the I2S sample rate: doing so while the AirPlay task is live
 * would change what that task's resampler targets.
 */
