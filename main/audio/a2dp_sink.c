...
// 队列**不能**浅: 事件是在 BTC 上下文里用 10ms 超时塞进来的 (那里不能长等,
// 否则整个蓝牙协议栈被卡住), 塞不进就是**静默丢弃**。而 bt_app_task 现在在
// 蓝牙连上那一刻要同步等 SD/电台两个本地音源退出 I2S, 最长能阻塞好几秒 ——
// 这段时间里如果 ESP_A2D_CONNECTION_STATE_EVT(DISCONNECTED) 被丢掉, s_connected
// 就永远停在 true, AirPlay 再也不会恢复 (main.c 的 on_airplay_client_event 一看见
// bt_a2dp_sink_is_connected() 就直接 return)。
