...
void audio_timing_init(audio_timing_t *timing, size_t pending_capacity) {
  ...
  if (pending_capacity > 0) {
    ...
    if (timing->pending_frame) {
      ...
      // 不是致命错误 (early 帧会被直接丢弃而不是暂存, 池子也能正常回收),
      // 但**必须看得见** —— 否则现场只会看到"时机不对/偶发卡顿"，没有任何线索
      // 指向这里少了一块暂存帧。
      ESP_LOGE(TAG, "暂存帧分配失败 (%zu 字节) —— 提前帧将无法暂存，播放时机退化",
               pending_capacity);
    }
  }
}
        ...
        if (early_us > release_us) {
          ...
          if (timing->consecutive_early_frames > MAX_CONSECUTIVE_EARLY) {
            ...
            if (!from_pending && timing->pending_frame &&
              ...
              // ⚠️ 存不成 pending 也必须把 item 还回池子。存不成的现实原因是
              //    `pending_frame == NULL` —— audio_timing_init() 那块 malloc
              //    失败时是静默容忍的 —— 或者这帧比 pending 容量还大。少了这一
              //    支，每个 early 帧都会吞掉一个池槽，free_top 走到 0 之后就
              //   再也 queue 不进来，表现为**永久静音**。
              //    (from_pending 时 item 指的是 timing->pending_frame 自己，
              //     那块不是池子里的，绝不能还。)
              audio_buffer_return(buffer, item);
            }
