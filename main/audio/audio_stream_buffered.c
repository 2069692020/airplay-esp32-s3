...
static void buffered_audio_task(void *pvParameters) {
  ...
  while (stream->running) {
    ...
    while (stream->running) {
      ...
      // ⚠️ 下面无条件读 packet[1..7] 取 seq/timestamp。包长不足 8 时那几个字节
      //    **这一轮根本没被写过**，拿到的是上一个包留下的残值 —— 于是一个假的
      //    RTP 时间戳会去喂"丢弃过期帧"那道判断 (可能把正常帧全丢掉)。真实的
      //    buffered AAC 包一定带这 8 字节头，所以这里不会误杀合法流量。
      if (packet_len < 8) {
        ESP_LOGW(TAG, "Buffered audio packet too short for header: %zu",
                 packet_len);
        break;
      }
