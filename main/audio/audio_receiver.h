...
/**
 * 注册 "AirPlay 即将占用 I2S" 的回调。
 *
 * AirPlay 会话建立 (ANNOUNCE/SETUP) 时调用, 让其他音源 (web radio 等)
 * 主动让出 I2S。用回调而不是直接调用, 是为了避免 main <-> 组件 的循环依赖:
 * main 依赖 web_radio, 所以 web_radio 不能反向 include main 的头。
 * 由 main.c 在启动时注册 (见 main.c 里 web_radio_yield_to_airplay 的注册)。
 *
 * 回调在 RTSP 处理任务的上下文里同步执行, 实现里不要长时间阻塞。
 */
typedef void (*audio_receiver_yield_cb_t)(void);

void audio_receiver_set_yield_callback(audio_receiver_yield_cb_t cb);

/**
 * 触发已注册的 yield 回调 (若有)。在 AirPlay 会话建立时调用。
 */
void audio_receiver_notify_yield(void);
