...
static void event_port_task(void *pvParameters) {
  ...
  while (!event_task_should_stop && listen_socket >= 0) {
    ...
    // ⚠️ 这个超时必须明显小于 rtsp_stop_event_port_task() 的等待预算
    //    (20 × 50ms = 1s)。shutdown() 打在 **listen** socket 上，lwIP 不保证能
    //    唤醒阻塞在 select() 上的任务 —— 原来这里也是 1s，和等待预算**一样长**，
    //    所以断开时那条 "Event port task did not exit within timeout" 是必然
    //    偶发的 (§25.8 记的那条)。200ms 留 5 倍余量。
    ...
    if (FD_ISSET(listen_socket, &read_fds)) {
      ...
      while (event_client_socket >= 0 && !event_task_should_stop) {
        ...
        // 同外层: 必须明显短于停止路径的 1s 预算
        struct timeval ctv = {.tv_sec = 0, .tv_usec = 200000};
...
static void handle_announce(int socket, rtsp_conn_t *conn,
  ...
  // 新 AirPlay 会话开始 — 让本地音源 (网络电台 / SD 卡) 让出 I2S。
  // 放在 ANNOUNCE (而不是等 RECORD) 是因为 ANNOUNCE 已经确定要推流了,
  // 早让出可以避免 AirPlay 的 SETUP/RECORD 和它们抢 I2S 的时序竞争。
  // ⚠️ 但**必须在 200 OK 之后**才调, 见下面说明。
  if (req->body && req->body_len > 0) {
    parse_sdp(conn, (const char *)req->body, req->body_len);
  }

  // ⚠️ 顺序: 先回 200 OK, 再让出 I2S (2026-09-18 修)。
  //
  //    notify_yield 是**同步**的: 它一路调到音源的 stop_and_yield, 而那里会
  //    等 play/reader 任务真正退出 (以前最长 5 秒)。这段时间全花在 RTSP 连接
  //    任务里, 而且是在发响应之前 —— 手机等不到 ANNOUNCE 的响应就会放弃这次
  //    推流, 表现就是"iPhone 推不过去"。
  //
  //    调换顺序不影响互斥: 同一条 RTSP 连接上的请求是**串行**处理的, SETUP
  //    必须等本函数返回之后才会被处理, 所以 I2S 仍然在 SETUP/RECORD 之前让出。
  rtsp_send_ok(socket, conn, req->cseq);
  audio_receiver_notify_yield();
}
...
static void handle_setup(int socket, rtsp_conn_t *conn,
  ...
  if (body && body_len > 0) {
    ...
    if (shk_len >= 16) {
      ...
      encryption_set = true;
               ...
               conn->hap_session->session_established) {
      uint8_t nonce[12] = {0};
      /* The AEAD writes (ekey_len - ABYTES) plaintext bytes, and ekey_len is
       * bounded by sizeof(ekey_encrypted) rather than by 32: a 64-byte ekey
       * yields 48 plaintext bytes and would overrun a 32-byte buffer.  Size
       * this from the ciphertext buffer so the two cannot drift apart. */
  ...
  // ⚠️ ADD (2026-09-18): 流 SETUP 也要让本地音源让出 I2S。
  //    正常 v2 推流在 RECORD 那一步已经让过了 (见 handle_record), 这里是兜底:
  //    代码里另一条路径 —— "流 TEARDOWN 之后只发一个新的 SETUP 就恢复播放"
  //    (见下面 RECORD 附近的注释) —— 不发 RECORD。若那时 SD/电台正在播,
  //    没有这一句就又是"歌在播放但没声音"。
  //    重复触发安全: main.c 的 s_airplay_owns_i2s 保证只真正交还一次。
  //    只在这一支 (真正的流 SETUP) 调, 不在上面 has_streams=0 的初始连接里调 ——
  //    后者只是手机打开了个会话 (比如在 AirPlay 菜单里看了一眼), 不该打断本地播放。
  audio_receiver_notify_yield();
  ...
  dac_set_volume(airplay_get_volume_db());
#endif
...
static void handle_record(int socket, rtsp_conn_t *conn,
  ...
  // ⚠️ ADD (2026-09-18): RECORD 也要让本地音源让出 I2S —— 这是 **AirPlay 2**
  //    推流时最早、也最可靠的信号。
  //
  //    起因: SD 卡播放中投 AirPlay, 推不过去。实测日志 (见 §25) 里, 这台
  //    iPhone 的 v2 会话从头到尾**没有 ANNOUNCE**:
  //        New client connected -> SETUP(has_streams=0) -> RECORD
  //        -> SETPEERS -> SETUP(has_streams=1, Configured codec ALAC) -> ...
  //    codec 是塞在 SETUP 的 bplist 里带来的, 所以挂在 handle_announce 上的
  //    yield 一次都没被调用 —— SD 一直占着 I2S, playback_task 停着, AirPlay
  //    的数据只能躺在缓冲里没人写, 直到用户手动停止 SD 才出声。
  //    (§21 那次的修复是在 v1 上验证的; §24 当天切回 v2 之后这条就失效了。)
  //
  //    放在 200 OK 之后 (理由同 handle_announce): 让出是同步的, 不能让手机干等。
  //    重复触发是安全的 —— main.c 里 s_airplay_owns_i2s 保证只真正交还一次。
  audio_receiver_notify_yield();
}
    ...
    if (body) {
      if (strstr((const char *)body, "volume:")) {
        ...
        if (vol) {
          ...
          // 音量是设备级状态，不再存在 conn 上 (见 rtsp_server.h)
          airplay_apply_volume_db(volume);
        }
      }
