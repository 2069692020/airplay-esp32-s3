...
// Advertised to the sender as `audioBufferSize` in the AirPlay 2 SETUP
// response; the sender fills no further ahead than this, so it sets a floor on
// latency.  Tunable via CONFIG_AIRPLAY_AUDIO_BUFFER_KB (see the Kconfig help
// for the KiB -> seconds conversion and the latency/robustness trade-off).
#define AP2_AUDIO_BUFFER_SIZE (CONFIG_AIRPLAY_AUDIO_BUFFER_KB * 1024)
