...
static bool bplist_parse_trailer(const uint8_t *plist, size_t plist_len,
  ...
  *offset_table_offset = read_be_int(trailer + 24, 8);
  ...
  /* Bound num_objects by what the offset table can actually hold: the table
   * starts at offset_table_offset and is num_objects * offset_size bytes long.
   * Without this a crafted trailer sets num_objects to ~2^64 and the
   * `for (i < num_objects)` object walks spin over astronomically many
   * entries.  top_object is an index into that table and must be in range too.
   * Every valid plist satisfies both (the table has exactly one entry per
   * object), so this cannot reject legitimate input. */
  uint64_t table_capacity =
      (plist_len - *offset_table_offset) / *offset_size;
  if (*num_objects == 0 || *num_objects > table_capacity ||
...
/* Reads entry `obj_idx` of the offset table, returning UINT64_MAX when the
 * entry cannot name an object in the buffer — either because `obj_idx` lies
 * outside the table, or because the stored offset itself is out of range.
 *
 * The bound is load-bearing twice over:
 *  - in the reference-driven paths obj_idx is read straight out of the file
 *    (up to 2^64-1), so without the index check the multiply below forms a wild
 *    pointer and the read in read_be_int() IS the out-of-bounds access — before
 *    any caller gets a chance to validate the result.
 *  - the offset table's *contents* are file input too.  Validating only the
 *    slot we read from would let a crafted table hand back an arbitrary offset,
 *    and not every caller re-checks it before dereferencing.  Making the
 *    sentinel mean "never a valid index" is what lets the comment below hold.
 *
 * Every entry of a well-formed plist points at that object's marker byte, so it
 * is strictly less than plist_len — this can never reject legitimate input. */
static uint64_t bplist_get_offset(const uint8_t *plist, size_t plist_len,
                                  uint64_t offset_table_offset,
                                  uint8_t offset_size, uint64_t obj_idx) {
  if (offset_table_offset > plist_len) {
    return UINT64_MAX;
  }
  /* obj_idx < entries implies (obj_idx + 1) * offset_size <= table extent, so
   * the multiply below cannot overflow and the entry is fully in bounds. */
  size_t entries = (plist_len - (size_t)offset_table_offset) / offset_size;
  if (obj_idx >= (uint64_t)entries) {
    return UINT64_MAX;
  }
  const uint8_t *entry = plist + offset_table_offset + obj_idx * offset_size;
  uint64_t offset = read_be_int(entry, offset_size);
  if (offset >= plist_len) {
    return UINT64_MAX;
  }
  return offset;
...
static bool bplist_read_string(const uint8_t *plist, size_t plist_len,
  ...
  if (type == BPLIST_UNICODE) {
    size_t bytes = len * 2;
    if (bytes > plist_len - pos || len >= out_capacity) {
      return false;
    }
...
static bool bplist_find_data_in_dict(const uint8_t *plist, size_t plist_len,
  ...
  if (dict_size > (plist_len - pos) / (2 * ref_size)) {
    return false;
  }
...
static bool bplist_find_data_recursive(const uint8_t *plist, size_t plist_len,
  ...
  if (type == BPLIST_DICT) {
    ...
    size_t pos = offset + header_len;
    if (count > (plist_len - pos) / ref_size) {
      return false;
    }
  ...
  *type = -1;
  ...
  for (size_t i = 0; i < dict_size; i++) {
    ...
    if (key_offset == streams_key_offset) {
      ...
      if (array_count > (plist_len - array_pos) / ref_size) {
        return false;
      }
      ...
      if (stream_dict_size > (plist_len - stream_pos) / (2 * ref_size)) {
        return false;
      }
