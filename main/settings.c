...
#define NVS_KEY_OTA_PASSWORD   "ota_pass"
...
/* ── 固件升级口令 ─────────────────────────────────────────────────────────
 * 空串是一个**有效值**，含义是"网页升级接口关闭"。所以这个 getter 和其它
 * 几个不一样：没有默认值可回退，找不到键就返回空串。绝不把口令写进日志。
 * 见 web_server.c: ota_update_handler() —— 口令为空 ⇒ 直接 403。
 */

esp_err_t settings_get_ota_password(char *out, size_t len) {
  if (!out || len == 0) {
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (err == ESP_OK) {
    size_t required_size = len;
    err = nvs_get_str(nvs, NVS_KEY_OTA_PASSWORD, out, &required_size);
    nvs_close(nvs);
    if (err != ESP_OK) {
      out[0] = '\0'; // 没设过 = 关闭，不是错误
...
esp_err_t settings_set_ota_password(const char *password) {
  if (!password || strlen(password) > SETTINGS_OTA_PASSWORD_MAX) {
    return ESP_ERR_INVALID_ARG; // 空串合法：表示关掉网页升级
  }
  ...
  err = nvs_set_str(nvs, NVS_KEY_OTA_PASSWORD, password);
  if (err == ESP_OK) {
    err = nvs_commit(nvs);
  }
  ...
  // 只报"有没有设"，不报内容
  ESP_LOGI(TAG, "升级口令%s", password[0] ? "已更新" : "已清空（网页升级关闭）");
